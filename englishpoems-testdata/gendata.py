...
def getPoem():
    cmd = subprocess.Popen(["./basic-english.pl", '128'], stdout=subprocess.PIPE)
    cmd_out, cmd_err = cmd.communicate()
    return cmd_out
    
f = open("word-lists/basic-adjectives", 'r')
adjectives = [ l.strip() for l in f.readlines()]
f = open("word-lists/basic-adverbs", 'r')
adverbs = [ l.strip() for l in f.readlines()]
f = open("word-lists/basic-articles", 'r')
articles = [ l.strip() for l in f.readlines()]
f = open("word-lists/basic-nouns", 'r')
nouns = [ l.strip() for l in f.readlines()]
f = open("word-lists/basic-prepositions", 'r')
prepositions = [ l.strip() for l in f.readlines()]
f = open("word-lists/basic-verbs-i", 'r')
verbsi = [ l.strip() for l in f.readlines()]
f = open("word-lists/basic-verbs-t", 'r')
